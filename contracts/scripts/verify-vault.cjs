...
async function main() {
  ...
  const [asset, maturityPeriod, rewardFundingCooldown, targetApyBps, programRewardBps, rewardSource] = await Promise.all([
    vault.asset(),
    vault.MATURITY_PERIOD(),
    vault.REWARD_FUNDING_COOLDOWN(),
    vault.TARGET_APY_BPS(),
    vault.PROGRAM_REWARD_BPS(),
    pool.rewardSource(),
  ])
  ...
  if (programRewardBps !== 295n) throw new Error("Unexpected program reward bps")
  ...
  console.log(JSON.stringify({
    ...
    programRewardBps: Number(programRewardBps),
    apyAccounting: true,
