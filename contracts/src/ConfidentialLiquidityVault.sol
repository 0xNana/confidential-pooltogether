...
contract ConfidentialLiquidityVault is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    uint64 public constant PROGRAM_REWARD_BPS = 295;
    ...
    /// @notice Contributes the 90-day 12% APY target from the reward reserve to the prize pool.
    /// @dev Uses 295 bps, the rounded-down 90-day portion of a 1200 bps annual target.
    function fundPrizePool(address prizePool) external onlyOwner nonReentrant {
        ...
        euint64 targetReward = FHE.div(FHE.mul(_totalPrincipal, PROGRAM_REWARD_BPS), 10_000);
        euint64 payout = FHE.min(_rewardReserve, targetReward);
